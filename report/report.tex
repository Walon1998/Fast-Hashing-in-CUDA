...
To measure the performance of the kernels \emph{nvprof} \cite{nvprof} was used with the following options: \texttt{--con\-current-\-kernels off --csv --profile\--from\--start off --print\--gpu\--trace }. The  turbo  boost  of  the  accelerators  were  disabled  using \texttt{export CUDA\_AUTO\_BOOST=0}. As host compiler gcc \cite{gcc} version 10.1 was used with the following flags: \texttt{-O3 -march=native -std=c++14}. Host code was measured using \texttt{std\-::\-chrono\-::\-high\-\_\-resolution\-\-\_\-clock}. 
...
In CUDA, it is possible to use the \emph{funnel shift} instruction to implement rotations, which reduces the instruction count drastically. In our main loop there are 1982 arithmetic instruction, that contribute to hashing, and only 22 non arithmetic instructions, which mean our implementation can reach up to 98.9\% of the reachable peak performance. Note that we compare against reachable peak performance and not \emph{real} peak performance, since in order to achieve real peak performance one has to use instructions that have a higher throughput than regular instructions (e.g. Fused Multiply Add), but these do not appear in SHA-256.
