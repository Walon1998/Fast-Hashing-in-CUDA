...
\mypar{Block PARSHA-256} If $t \leq 10$ at most 1024 threads are launched, which means the whole calculation can fit into in a single block, see Fig. \ref{fig:block}. In that case shared memory and \emph{\_\_syncthreads()} can be used to communicate and synchronize between the warps. Note that in this case, the property that a thread with the id $i$ has two children with the id $2i$ and $2i+1$ no longer applies, but a more complicated relationship is required so that communication across warps is minimized.
...
To measure the performance of the kernels \emph{nvprof} \cite{nvprof} was used with the following options: \texttt{--con\-current \-kernels off --csv --profile\--from\--start off --print\--gpu\--trace --profile\--api\--trace all }. As host compiler gcc \cite{gcc} version 10.1 was used with the following flags: \texttt{-O3 -march=native}.
Note that only the time taken to hash the padded message was measured, but the time taken to preprocess the message was ignored because it is the same for every message.
...
To collect performance results, each string was hashed 110 times with the first 10 iterations being warm-up rounds, where the kernel time was not measured. In the next 100 iterations the kernel time was measured.
